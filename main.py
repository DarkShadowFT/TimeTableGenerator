...
def latestWorkBook():
    newTT = True
    tt_files = []
    for root, directory, files in os.walk("C:\Data\SHIT-NUCES\Current Semester"):
        for file in files:
            if ((file.lower().find("fast school of computing time table") == 0) or
                (file.lower().find("fsc time table") == 0)) and file.lower().find("~$") == -1:
...
with open("tt version.txt", "r") as ttv:
    path = ttv.read()
wb = Book(path, read_only=True)

# TODO: Find a way to get the required sheet by index instead of name as name keeps changing
sheet_name = 'FSC TT (Fall 2022)'
...
# TODO: Need to update these sections too
semester_number = 7
sections = [f"BCS-{semester_number}A", f"BCS-{semester_number}B", f"BCS-{semester_number}C", f"BCS-{semester_number}D",
            f"BCS-{semester_number}E", f"BCS-{semester_number}F", f"BCS-{semester_number}G", f"BCS-{semester_number}H",
            f"BCS-{semester_number}J"]
...
my_choices = [["Information Security", "BCS-7A"], ["Professional Practices", "BCS-7C"],
              ["Data Warehousing and Data Mining", "BCS-7A"], ["Software for Mobile Devices", "BCS-7B"]]
my_courses = ["Information Security", "Professional Practices", "Data Warehousing and Data Mining",
              "Software for Mobile Devices"]
my_courses_short = ["IS", "PP", "SMD", "DM"]
...
# dest = shutil.copyfile('C:\Data\SHIT-NUCES\Current Semester\TimeTable.png', 'C:\Data\SHIT-NUCES\Current
# Semester\TimeTable(Copy).png')
dest = shutil.copyfile('timetable.png', 'C:\Data\SHIT-NUCES\Current Semester\TimeTable.png')
# dest = shutil.copyfile('C:\Data\SHIT-NUCES\Current Semester\TimeTable.xlsx', 'C:\Data\SHIT-NUCES\Current
# Semester\TimeTable(Copy).xlsx')
